...
const expirationMask = IMask(expirationDate, expirationDatePattern)
...
const addButton = document.querySelector("#adds-button")
...
securityMask.on("accept", () => {
  updatedSecurityCode(securityMask.value)
})
...
expirationMask.on("accept", () => {
  updateExpirationMask(expirationMask.value)
})
function updateExpirationMask(date) {
